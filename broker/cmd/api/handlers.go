...
func (app *Config) LogItemGRPC(w http.ResponseWriter, load LogPayload) {
	conn, err := grpc.Dial("logger:5032", grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
	if err != nil {
		app.errorJson(w, err)
		return
	}
